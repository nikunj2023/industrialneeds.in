...
<form id="bbp-your-profile" action="<?php bbp_user_profile_edit_url( bbp_get_displayed_user_id() ); ?>" method="post" enctype="multipart/form-data">
	...
	<?php do_action( 'bbp_user_edit_before' ); ?>
		...
		<section>
			<label for="user_login" class="lbl-text"><?php _e( 'Username', 'pointfinder' ); ?></label>
		...
		<section>
			<label for="email" class="lbl-text"><?php _e( 'Email', 'pointfinder' ); ?></label>
			...
			if ( !empty( $new_email ) && $new_email !== bbp_get_displayed_user_field( 'user_email', 'edit' ) ) : ?>
				...
				<span class="updated inline">
					...
					<?php printf( __( 'There is a pending email address change to <code>%1$s</code>. <a href="%2$s">Cancel</a>', 'pointfinder' ), $new_email['newemail'], esc_url( self_admin_url( 'user.php?dismiss=' . bbp_get_current_user_id()  . '_new_email' ) ) ); ?>
		...
		<section id="password">
			<label for="pass1" class="lbl-text"><?php _e( 'New Password', 'pointfinder' ); ?></label>
			<fieldset class="bbp-form password">
				...
				<span class="description"><?php _e( 'If you would like to change the password type a new one. Otherwise leave this blank.', 'pointfinder' ); ?></span>
				</section>
				<section>
				<label class="lbl-ui">
				<input type="password" name="pass2" class="input" id="pass2" size="16" value="" autocomplete="off" tabindex="<?php bbp_tab_index(); ?>" />
				</label>
				<span class="description"><?php _e( 'Type your new password again.', 'pointfinder' ); ?></span><br />
				</section>
				<section>
				<div id="pass-strength-result"></div>
				<span class="description indicator-hint"><?php _e( 'Your password should be at least ten characters long. Use upper and lower case letters, numbers, and symbols to make it even stronger.', 'pointfinder' ); ?></span>
				</section>
			</fieldset>
		</section>

		<section>
			<label for="first_name" class="lbl-text"><?php _e( 'First Name', 'pointfinder' ) ?></label>
			<label class="lbl-ui">
		...
		<section>
			<label for="last_name" class="lbl-text"><?php _e( 'Last Name', 'pointfinder' ) ?></label>
			<label class="lbl-ui">
		...
		<section>
			<label for="nickname" class="lbl-text"><?php _e( 'Nickname', 'pointfinder' ); ?></label>
			<label class="lbl-ui">
		...
		<section>
			<label for="display_name" class="lbl-text"><?php _e( 'Display Name', 'pointfinder' ) ?></label>
			<label class="select">
		...
		<section>
			<label for="description" class="lbl-text"><?php _e( 'Biographical Info', 'pointfinder' ); ?></label>
	...
	<div class="form-enclose clearfix"><div class="form-section">
		...
		<section>
			<label for="url" class="lbl-text"><?php _e( 'Website', 'pointfinder' ) ?></label>
		...
		<?php if ( current_user_can( 'edit_users' ) && ! bbp_is_user_home_edit() ) : ?>
				...
				<?php if ( is_multisite() && is_super_admin() && current_user_can( 'manage_network_options' ) ) : ?>
					...
					<section>
						<label for="super_admin"><?php _e( 'Network Role', 'pointfinder' ); ?></label>
						<label>
							...
							<?php _e( 'Grant this user super admin privileges for the Network.', 'pointfinder' ); ?>
						</label>
					</section>
		...
		<section>
			...
			<button type="submit" tabindex="<?php bbp_tab_index(); ?>" id="bbp_user_edit_submit" name="bbp_user_edit_submit" class="button submit user-submit"><?php bbp_is_user_home_edit() ? _e( 'Update Profile', 'pointfinder' ) : _e( 'Update User', 'pointfinder' ); ?></button>
		</section>
