...
if (!class_exists('pf_singlepost_info')) {
    function pf_singlepost_info(){
        ...
        echo '<div class="post-minfo"><i class="far fa-calendar-alt"></i>';
        echo get_the_time(get_option('date_format'));
        
       
      
        $author_name = get_the_author_posts_link();
        echo '<i class="pf-pl-10 far fa-user"></i>'.$author_name;

        if (post_password_required()) {
          echo '<i class="pf-pl-10 far fa-comments"></i>'.esc_html__( 'Password Required', 'pointfinder' );
        }else{
          echo comments_popup_link( '<i class="pf-pl-10 far fa-comments"></i>'.esc_html__( 'No comments', 'pointfinder' ), '<i class="pf-pl-10 far fa-comments"></i>'.esc_html__( '1 Comment', 'pointfinder' ), '<i class="pf-pl-10 far fa-comments"></i>'.esc_html__( '% Comments', 'pointfinder' ),'', '<i class="pf-pl-10 far fa-comments"></i>'.esc_html__( 'Comments Off', 'pointfinder' ));  
        }


        $category_list = get_the_category_list( ', ' );
        echo '<i class="pf-pl-10 far fa-folder"></i>'.$category_list;

        $tags_list = pointfinder_show_post_tags();
        if (!empty($tags_list)) {
            echo '<i class="pf-pl-10 fas fa-tag"></i>'.$tags_list;
        }
...
if (!class_exists('pf_singlepost_info_list')) {
    function pf_singlepost_info_list(){
        ...
        echo '<div class="meta-comment-link pf-comment-rm-link">';
            echo '<a class="post-link" href="'.get_the_permalink().'" title="'.get_the_title().'">'.esc_html__('Read more','pointfinder').'&nbsp;<i class="fas fa-angle-right"></i></a>';
        echo '</div>';
    }
}

if (!function_exists('pointfinder_show_post_tags')) {
    function pointfinder_show_post_tags()
    {   
        $output = '';
