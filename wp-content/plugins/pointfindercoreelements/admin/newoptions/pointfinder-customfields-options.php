...
$setup1_slides = $PointFinderNewOptionsHelper->get_custom_fields_unfiltered();
$pfstart = $PointFinderNewOptionsHelper->get_custom_fields();
	...
	foreach ($setup1_slides as $value) {
		if (!empty($value['url'])) {
			if($value['select'] != 10 && $value['select'] != 16){
				$sections[] = $PointFinderNewOptionsHelper->CDF($value['title'],$value['url'],$value['select']);
			}
		}
