    ...
    if ( ! class_exists( 'ReduxFramework_extension_metaboxes' ) ) {
        ...
        class ReduxFramework_extension_metaboxes {
                        ...
                        foreach ( $box['post_types'] as $posttype ) {
                            if ( isset( $box['title'] ) ) {
                                ...
                                if ( isset( $box['sections'] ) && count( $box['sections'] ) == 1 && isset( $box['sections'][0]['fields'] ) && count( $box['sections'][0]['fields'] ) == 1 && isset( $box['sections'][0]['fields'][0]['title'] ) ) {
                                    ...
                                    $title = ucfirst( $posttype ) . " " . __( 'Options', 'pointfindercoreelements' );
                                }
            ...
            function generate_boxes( $post, $metabox ) {
                ...
                ?>
                <div data-opt-name="<?php echo esc_attr($this->parent->args['opt_name']);?>" class="redux-container<?php echo ( $sidebar ) ? ' redux-has-sections' : ' redux-no-sections'; ?> redux-box-<?php echo $metabox['args']['position']; ?>">
                    <div class="redux-notices">
                        <?php if ( $sidebar ) { ?>
                            <div class="saved_notice admin-notice notice-blue" style="display:none;">
                                <strong><?php echo apply_filters( "redux-imported-text-{$this->parent->args['opt_name']}", __( 'Settings Imported!', 'pointfindercoreelements' ) ) ?></strong>
                            </div>
                            <div class="redux-save-warn notice-yellow">
                                <strong><?php echo apply_filters( "redux-changed-text-{$this->parent->args['opt_name']}", __( 'Settings have changed, you should save them!', 'pointfindercoreelements' ) ) ?></strong>
                            </div>
                        <?php } ?>
                        <div class="redux-field-errors notice-red">
                            <strong> <span></span> <?php echo __( 'error(s) were found!', 'pointfindercoreelements' ) ?>
                            </strong>
                        </div>
                        <div class="redux-field-warnings notice-yellow">
                            <strong> <span></span> <?php echo __( 'warning(s) were found!', 'pointfindercoreelements' ) ?>
                            </strong>
                        </div>
                    </div>
                    <?php
                        echo '<a href="javascript:void(0);" class="expand_options hide" style="display:none;">' . __( 'Expand', 'pointfindercoreelements' ) . '</a>';
                        if ( $sidebar ) {
                            ?>
            ...
            function meta_boxes_show_errors() {
                if ( isset( $this->notices['errors'] ) && ! empty( $this->notices['errors'] ) ) {
                    ...
                    echo '<p><strong><span></span> ' . count( $this->notices['errors'] ) . ' ' . __( 'error(s) were found!', 'pointfindercoreelements' ) . '</strong></p>';
                    echo '</div>';
                }
                ...
                if ( isset( $this->notices['warnings'] ) && ! empty( $this->notices['warnings'] ) ) {
                    ...
                    echo '<p><strong><span></span> ' . count( $this->notices['warnings'] ) . ' ' . __( 'warnings(s) were found!', 'pointfindercoreelements' ) . '</strong></p>';
                    echo '</div>';
                }
