...
if (!trait_exists('PointFinderWPMLStringGenerator')) {
	/**
	 * WPML String Generator
	 */
	trait PointFinderWPMLStringGenerator
	{
	    
	    public function pf_register_wpml_string_output(){
	    	if (class_exists('Sitepress')) {
		    	add_submenu_page('pointfinder_tools', '', esc_html__('WPML Config','pointfindercoreelements'), 'switch_themes', 'pfwpmlstring', array($this,'pf_wpml_string_output'));
		    }
		}

		public function pf_wpml_string_output(){
			
			if (!class_exists('Sitepress')) {
				echo '<p>';esc_html_e('Please activate WPML plugin then try again','pointfindercoreelements');echo '</p>';
				return;
			}
			
			echo '<p>';
			    echo esc_html__('Please follow steps below;','pointfindercoreelements');
			    echo '  <ol>
						  <li>'.esc_html__('Please finish to configure PF Settings > Options Panel, Custom Fields Config, Search Fields Config and Review System Config','pointfindercoreelements').'</li>
						  <li>'.esc_html__('Click to "GENERATE WPML STRINGS" button.','pointfindercoreelements').'</li>
						  <li>'.esc_html__('Copy below text code and paste to wp-content > pointfinder > wpml-config.xml','pointfindercoreelements').'</li>
						  <li>'.esc_html__('Then you can use WPML > String Translation.','pointfindercoreelements').'</li>
						</ol>
						<br/>
						'.esc_html__('Note: This system will create custom strings for WPML by using admin & field options configuration. Please re create this file after change dynamic fields.','pointfindercoreelements').'
					';
				...
				<input type="submit" name="publish" id="publish" class="button button-primary button-large" value="'.esc_html__( 'GENERATE WPML STRINGS', 'pointfindercoreelements' ).'" accesskey="p"></form>';
				echo '<br/><br/>'.esc_html__('On mouse click system will select all text into textarea. You only need to copy it.','pointfindercoreelements');
				...
				if (isset($_GET['wpmlgenerate'])) {
					echo '<wpml-config>'.PHP_EOL;
						...
						**/
							...
							if ($setup11_reviewsystem_check != 0) {
						...
						**/
						...
						/**
						*Start: PF Additional Options
						**/
							echo '<key name="pfascontrol_options">';
								echo '<key name="general_postitembutton_buttontext" />';
							echo '</key>';
						/**
						*End: PF Additional Options
						**/
						...
						**/
							echo '<key name="pointfindertheme_options">';
								...
								/* Theme Customizer */
				        			...
				        			<key name="setup19_socialiconsbarsettings_envelope_link" />
									';

								/* Footer Bar */
								
								echo '
						        	<key name="setup_footerbar_text_copy" />
				        			<key name="setup_footerbar_text_copy_align" />
									';
								/* Submission System */
									echo '
									...
									<key name="setup4_submitpage_maparea_title" />
									<key name="setup4_submitpage_maparea_tooltip" />
									';
								...
								/* Post Types */
						            ...
						            <key name="setup3_pt14s" />
									';

								/* Review Criterias */

								/* Map Settings */
									echo '<key name="setup12_searchwindow_mapinfotext" />
									<key name="setup5_mapsettings_maplanguage"/>
									<key name="heremapslang"/>
									<key name="wemap_langy"/>
									<key name="wemap_country"/>
									<key name="wemap_country3"/>
									';
									...
									if($pfstart){
										...
										foreach ($setup1_slides as &$value) {
											...
											if(in_array($value['select'], $exclude_list2)) {
												...
												$calc_rvalues = $this->PFCFIssetControl('setupcustomfields_'.$value['url'].'_rvalues','','');
												...
												echo '</key>';
												echo '<key name="setupcustomfields_'.$value['url'].'_frontendname" />
									            <key name="setupcustomfields_'.$value['url'].'_descriptionfront" />
									            <key name="setupcustomfields_'.$value['url'].'_sortname" />
									            <key name="setupcustomfields_'.$value['url'].'_shortname" />
									            <key name="setupcustomfields_'.$value['url'].'_description" />';
						...
						**/
							echo '<key name="pointfindermail_options">';
								...
								<key name="setup35_paymentmemberemails_bankpaymentapp_title" />
								<key name="setup35_paymentmemberemails_bankpaymentapp_subject" />
								<key name="setup35_paymentmemberemails_bankpaymentapp" />
						...
						**/
						...
						if ($st8_nasys != 1) {
						    ...
						    foreach ($terms as $term) {
								...
								echo '<key name="pfadvancedcontrol_options"><key name="setupadvancedconfig_'.$term->term_id.'_configuration">'.PHP_EOL;
										echo '
										<key name="gallery">
							            	<key name="title" />
							            </key>
					...
					echo esc_html__( 'Click to "GENERATE WPML STRINGS" button.', 'pointfindercoreelements' );
				}
				echo '</textarea>';
