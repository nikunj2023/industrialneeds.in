<?php 
if (!class_exists('PointFinderAjaxListingTypes')) {
	class PointFinderAjaxListingTypes extends Pointfindercoreelements_AJAX
	{
	    public function __construct(){}

	    public function pf_ajax_listingtype(){
			...
			if (!empty($id) && !empty($sname) && !empty($stext)) {
				...
				$output = $this->Pointfinder_GET_LType_FUP($fields_output_arr);
		...
		private function Pointfinder_GET_LType_FUP($params = array()){
		    $defaults = array(
		        'listname' => '',
		        'listtype' => '',
		        'listtitle' => '',
		    ...
		    $params = array_merge($defaults, $params);
				...
				if (count($fieldvalues) > 0) {
					...
					if (!empty($params['listtitle']) && $params['alt'] == 1) {
						$output .= '<label for="'.$params['listname'].'" class="lbl-text">'.$params['listtitle'].':</label>';
					}



					if ($params['as_mobile_dropdowns'] == 1 && $params['listmultiple'] != 1) {
						$as_mobile_dropdowns_text = 'class="pf-special-selectbox" data-pf-plc="'.esc_html__('Please select','pointfindercoreelements').'" data-pf-stt="false"';
					} else {
