<?php 
if (!class_exists('PointFinderOwnerChange')) {
	class PointFinderOwnerChange extends Pointfindercoreelements_AJAX
	{
	    public function __construct(){}

	    public function pf_ajax_createorder(){
			...
			/*Check if any order exist about this property?*/
				...
				$ordered_package_name = $stp31_up2_pn = $this->PFSAIssetControl('stp31_up2_pn','',esc_html__('Basic Listing Payment','pointfindercoreelements'));

				$order_detailed_price = array($stp31_up2_pn => $setup31_userpayments_priceperitem);

				$setup31_userpayments_timeperitem = $this->PFSAIssetControl('setup31_userpayments_timeperitem','','');
				$order_recurring = 0;
				...
				if(!empty($check_featured)){
					if($check_featured == 1){
						...
						$setup31_userpayments_titlefeatured = $this->PFSAIssetControl('setup31_userpayments_titlefeatured','',esc_html__('Featured Item','pointfindercoreelements'));
						$ordered_package_id = 1;
				...
				$pack_results = $this->pointfinder_calculate_listingtypeprice('',$check_featured,$ordered_package_id);
				...
				$this->PFCreateProcessRecord(
					array( 
						...
						'processname' => esc_html__('Order created by Admin (User change function)','pointfindercoreelements')
				    )
				);	
