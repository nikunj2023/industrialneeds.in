<?php 
if (!class_exists('PointFinderAutoCMPLT')) {
	class PointFinderAutoCMPLT extends Pointfindercoreelements_AJAX
	{
	    ...
	    public function pf_ajax_autocomplete(){
			...
			}elseif ($ftype == 'listingid') {
				$args['p'] = absint($searchword);
			...
			if ($tax_query) {
				...
				if ( $the_query->have_posts() ) :
					...
					while ( $the_query->have_posts() ) : $the_query->the_post();
						if ($ftype == 'title' || $ftype == 'description' || $ftype == 'title_description' || $ftype == 'listingid') {
