...
if (!class_exists('PointFinderPaymentSystem')) {
  class PointFinderPaymentSystem extends Pointfindercoreelements_AJAX
  {
      ...
      public function pf_ajax_paymentsystem(){
                ...
                $pointfinder_sub_order_change = esc_attr(get_post_meta( $result_id, 'pointfinder_sub_order_change', true ));
                  ...
                  case 'creditcardstripe':
                    ...
                    if ($pointfinder_sub_order_change == 1 && $otype == 1) {
                       ...
                       if (empty($pointfinder_order_listingpname)) {
                         $pointfinder_order_listingpname = esc_html__( "Additional Update", "pointfindercoreelements" );
                       }
                    }else{
                       $pointfinder_order_price = esc_attr(get_post_meta( $result_id, 'pointfinder_order_price', true ));
                       $pointfinder_order_listingpname = esc_attr(get_post_meta($result_id, 'pointfinder_order_listingpname', true)); 
                    ...
                    try {
                      ...
                      $sessionkey = $stripe->checkout->sessions->create([
                        ...
                        'success_url' => $setup4_membersettings_dashboard_link.$pfmenu_perout.'ua=myitems&st=sc&r='.$result_id,
                        'cancel_url' => $setup4_membersettings_dashboard_link.$pfmenu_perout.'ua=myitems&st=f&r='.$result_id,
                        ...
                        "metadata" => ["user_id" => $user_id, "item_id" => $item_post_id, "order_id" => $result_id, "otype" => $otype, "subchange" => $pointfinder_sub_order_change]
                      ]);

                      $sessionid = $sessionkey->id;
                      update_post_meta( $result_id, 'pointfinder_order_stripesession', $sessionid );
                  ...
                  case 't2co':
                    ...
                    if ($t2cho_mode == 0) {
                      ...
                      $demomode = "<input type='hidden' name='demo' value='Y' />";
                    }else{
                      $url = "https://www.2checkout.com/checkout/purchase";
                      $demomode = '';
                    }
        ...
        $output_html .= "<div class='pf-overlay-close'><i class='fas fa-times'></i></div>";
        ...
        if ($icon_processout == 485) {  
          ...
          if ($formtype != 'payu' && $formtype != 'creditcardstripe' && $formtype != 'robo' && $formtype != 't2co' && $formtype != 'iyzico' && $formtype != 'payf') {
            ...
            echo json_encode( array( 'process'=>true, 'otype'=>$otype, 'name'=>$setup20_stripesettings_sitename, 'description'=>$pointfinder_order_listingpname, 'amount' => $total_package_price,'key'=>$setup20_stripesettings_publishkey,'email'=>$user_email,'currency'=>$setup20_stripesettings_currency, 'session'=>$sessionid, 'returnurl' => $pfreturn_url));
