<?php 
if (!class_exists('PointFinderAJAXFileSYS')) {
	class PointFinderAJAXFileSYS extends Pointfindercoreelements_AJAX
	{
	    public function __construct(){}

	    public function pf_ajax_filesystem(){
					...
					if ($id != '') {
						...
						if ($this->PFControlEmptyArr($files_of_thispost)) {
							...
							foreach ($files_of_thispost as $file_number) {
								$file_src_link = wp_get_attachment_url($file_number);//sprintf(esc_html__('Uploaded File (%d)','pointfindercoreelements'),$i)
							...
							$output .= '<section class="pfuploadform-mainsec">';
								$output .= '<label for="file" class="lbl-text">'.esc_html__('UPLOADED FILES','pointfindercoreelements').':</label>';
