<?php 
if (!trait_exists('PointFinderSocialLogins')) {
	/**
	 * Social Login module
	 */
	trait PointFinderSocialLogins
	{
		public function PF_SocialErrorHandler(){
			...
			if(!empty($pferror)){
				switch ($pferror) {
					case 'fbem':
						$pferror_text = wp_sprintf(esc_html__('Please complete %s Api setup from Admin Panel first.','pointfindercoreelements'),esc_html__('Facebook','pointfindercoreelements'));
						$pftype = 0;
						break;
					case 'tbem':
						$pferror_text = wp_sprintf(esc_html__('Please complete %s Api setup from Admin Panel first.','pointfindercoreelements'),esc_html__('Twitter','pointfindercoreelements'));
						$pftype = 0;
						break;

					case 'gbem':
						$pferror_text = wp_sprintf(esc_html__('Please complete %s Api setup from Admin Panel first.','pointfindercoreelements'),esc_html__('Google+','pointfindercoreelements'));
						$pftype = 0;
						break;
					case 'vbem':
						$pferror_text = wp_sprintf(esc_html__('Please complete %s Api setup from Admin Panel first.','pointfindercoreelements'),esc_html__('VK','pointfindercoreelements'));
						$pftype = 0;
						break;

					case 'gbue':
						$pferror_text = wp_sprintf(esc_html__('Can not login with %s','pointfindercoreelements'),esc_html__('Google+','pointfindercoreelements'));
						$pftype = 0;
						break;
					case 'vbue':
						$pferror_text = wp_sprintf(esc_html__('Can not login with %s','pointfindercoreelements'),esc_html__('VK','pointfindercoreelements'));
						$pftype = 0;
						break;


					case 'fbux':
						$pferror_text = esc_html__('The user can not found in our system. Login not completed.','pointfindercoreelements');
						$pftype = 0;
						break;
					...
					case 'tbem2':
						$pferror_text = esc_html__('Could not connect to the Twitter. Please try again later.','pointfindercoreelements');
						$pftype = 0;
						break;
					case 'tbem3':
						$pferror_text = esc_html__('The session is old. Please close/reopen your browser and try again.','pointfindercoreelements');
						$pftype = 0;
						break;
					case 'tbem4':
						$pferror_text = esc_html__('We could not verify your twitter account.','pointfindercoreelements');
						$pftype = 0;
						break;

					default:
						$pferror_text = esc_html__('No information','pointfindercoreelements');
						$pftype = 0;
						break;
				}
		...
		public function PF_SocialModalHandler(){
			if(isset($_GET['ctype'])){
		...
		public function PointFinder_Social_Facebook_Login(){
		        ...
		        switch ($redfreg) {
		          ...
		          case '3':
		            ...
		            break;
		          case '4':
		            $special_linkurl = $setup4_membersettings_dashboard_link.$pfmenu_perout.'ua=newitem';
		            break;
		          case '5':
		            if (!empty($redfregc)) {
		              $special_linkurl = $redfregc;
		            }else{
		              $special_linkurl = $homeurllink;
		            }
		            break;
				...
				if(isset($ua_action)){
					...
					**/
					if ($ua_action == 'fblogin') {
						...
						$setup4_membersettings_facebooklogin_appid = $this->PFASSIssetControl('setup4_membersettings_facebooklogin_appid','','');
						$setup4_membersettings_facebooklogin_secretid = $this->PFASSIssetControl('setup4_membersettings_facebooklogin_secretid','','');
					...
					**/
					if ($ua_action == 'twlogin') {
						...
						$setup4_membersettings_twitterlogin_appid = $this->PFASSIssetControl('setup4_membersettings_twitterlogin_appid','','');
						$setup4_membersettings_twitterlogin_secretid = $this->PFASSIssetControl('setup4_membersettings_twitterlogin_secretid','','');
					...
					**/
					if ($ua_action == 'gologin') {
						...
						$setup4_membersettings_googlelogin_clientid = $this->PFASSIssetControl('setup4_membersettings_googlelogin_clientid','','');
						$setup4_membersettings_googlelogin_secretid = $this->PFASSIssetControl('setup4_membersettings_googlelogin_secretid','','');
					...
					**/
					if ($ua_action == 'vklogin') {
						...
						$setup4_membersettings_vklogin_clientid = $this->PFASSIssetControl('setup4_membersettings_vklogin_clientid','','');
				...
				**/
				if(isset($_GET['code']) && isset($_GET['ltype']) && $this->PFASSIssetControl('setup4_membersettings_vklogin','','0') == 1) {

				    $setup4_membersettings_vklogin_clientid = $this->PFASSIssetControl('setup4_membersettings_vklogin_clientid','','');
					$setup4_membersettings_vklogin_secretid = $this->PFASSIssetControl('setup4_membersettings_vklogin_secretid','','');
				    ...
				    if (isset($request['body'])) {
				    	...
				    	if (isset($get_vk_body['access_token'])) {
				    		...
				    		$access_return_values = wp_remote_get("https://api.vk.com/method/getProfiles?v=5.74&access_token=".$get_vk_body['access_token']);
				    		...
				    		if(isset($access_return_values['body'])) {
						        ...
						        if (is_array($user_profile) && isset($user_profile['id'])) {
									...
									if ( empty($resultid) ) {
						        		$resultid = $wpdb->get_var( $wpdb->prepare("SELECT user_id FROM $wpdb->usermeta WHERE meta_key = %s and meta_value = %s", 'user_socialloginidvk','vk'.$user_profile['id']) );
						        	}
									...
									wp_redirect($special_linkurl);exit;
									    ...
									    $username = sanitize_user( 'vk'.$user_profile['id'], $strict = true );

										$scontenttext = array('email'=>'','username'=>$username,'dbid'=>'vk'.$user_profile['id']);
				    		...
				    		echo esc_html__('Error:','pointfindercoreelements' );
				    		if (isset($get_gogle_body['error'])) {
				    			echo $get_gogle_body['error'];
				    		}
				    		if (isset($get_gogle_body['error_description'])) {
				    			echo '<br/>'.esc_html__('Description:','pointfindercoreelements').' '.$get_gogle_body['error_description'];
				    		}
				    		wp_die();
				    	}
				    }
				...
				**/
				if(isset($_GET['code']) && !isset($_GET['ltype']) && $this->PFASSIssetControl('setup4_membersettings_googlelogin','','0') == 1) {
