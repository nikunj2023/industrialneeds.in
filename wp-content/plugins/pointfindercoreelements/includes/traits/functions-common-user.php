	...
	*/
	public function PFExpireItemManualMember($params){
		...
		*/
	    	...
	    	$json_array = json_encode($user_posts,JSON_UNESCAPED_UNICODE);
				...
				switch ($key) {
					case 'featured':
						if ($value == 1) {
							...
							update_post_meta($params['order_id'], 'pointfinder_order_featured', 1);
							...
							if (!class_exists('Pointfinderusersubscriptions',false)) {
								...
								$pointfinder_sub_order_featuredspupdate = get_post_meta( $params['order_id'], 'pointfinder_order_featuredspupdate',true );
								if (empty($pointfinder_sub_order_featuredspupdate)) {
									update_post_meta( $params['order_id'], 'pointfinder_order_frecurring', 0);
								}else{
									delete_post_meta( $params['order_id'], 'pointfinder_order_featuredspupdate' );
								}
							}

							$setp_renew_datef = $this->PFSAIssetControl('setp_renew_datef','','0');
							if ($setp_renew_datef) {
					...
					case 'plan':
						if ($value == 1) {
							...
							$setp_renew_date = $this->PFSAIssetControl('setp_renew_date','','0');
							if ($setp_renew_date) {
								wp_update_post( array('ID' => $params['post_id'], 'post_date' => $app_date) );
							}
	...
	*/
	public function pointfinder_reenable_expired_items($params = array()){
      ...
      if ($this->pf_membership_expire_check($membership_user_expiredate) == false) {
        $membership_user_expiredate_out = date_i18n("d-m-Y H:i:s",$membership_user_expiredate);
        if($params['process'] == 'r'){
