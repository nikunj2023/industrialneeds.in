...
$el_class = $this->getExtraClass($el_class);
    ...
    $el_end = '</div>';
    $slides_wrap_start = '<div class="pfslides owl-carousel owl-theme" id="'.$myrandno.'">';
    $slides_wrap_end = '</div>';
...
while ( $my_query->have_posts() ) {
    ...
    // if ( $thumbnail == '' ) $thumbnail = esc_html__("No Featured image set.", "pointfindercoreelements");

    // Link logic
    if ( $link != 'link_no' ) {
        if ( $link == 'link_post' ) {
            $link_image_start = '<a class="link_image" href="'.get_permalink($post_id).'" title="'.sprintf( esc_html__( 'Permalink to %s', 'pointfindercoreelements' ), the_title_attribute( 'echo=0' ) ).'">';
        }
            ...
            $link_image_start = '<a class="link_image pf-mfp-image" href="'.$p_link.'" title="'.the_title_attribute('echo=0').'" >';
        }
...
(function($) {
	...
	$(function() {
		$("#'.$myrandno.'").owlCarousel({
				items : 1,';
				if($hide_prev_next_buttons !== 'yes'){ $output .= 'nav : true,';}else{$output .= 'nav : false,';}
				if($hide_pagination_control !== 'yes'){ $output .= 'dots : true,';}else{$output .= 'dots : false,';}
				if($autoplay === 'yes'){ $output .= 'autoplay : true,autoplayHoverPause : true,';}else{$output .= 'autoplay : false,';}
				$output .= 'autoplayTimeout:'.$interval.',
				autoHeight : false,
                lazyLoad: false,
                loop: true,
                margin: 10,
